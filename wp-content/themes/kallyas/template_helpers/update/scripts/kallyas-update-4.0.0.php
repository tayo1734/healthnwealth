...
function zn_cnv_v4_convert_theme_options(){
	...
	$admin_options = ZNHGTFW()->getComponent('utility')->get_theme_options();
	...
	if( isset( $theme_options['zn_custom_css'] ) ){
		update_option( 'zn_'.ZNHGTFW()->getThemeId().'_custom_css', $theme_options['zn_custom_css'], false );
	}
	...
	foreach( $posts_data['posts'] as $meta_data )
	{
		...
		if ( $meta_values )
		{
			...
			foreach ( $meta_values as $area_name => &$value )
			{
				...
				elseif( in_array( $area_name, $pb_content_areas ) )
				{
					...
					foreach ( $meta_values[$area_name] as $i => &$element) {
						...
						if(! is_null($element)){
							$el = ZNB()->frontend->addModuleToLayout( $element['object'], $element['options'] );
							$columns[] = ZNB()->frontend->addModuleToLayout( 'ZnColumn', array() , array( $el ), $element['width'] );
						}
						...
						if ( $size == '1' || $size == '0.99' || $size == '0.91' || $size == '0.88' ) {
							$spacer_element = ZNB()->frontend->addModuleToLayout( 'TH_SpacerElement', array( 'spacer_height' => '35' ) , array(), false );
							$columns[] = ZNB()->frontend->addModuleToLayout( 'ZnColumn', array() , array( $spacer_element ), 'col-sm-12' );
							$size = 0;
						}
			...
			if( ! $has_cusotm_sub_header && ( !empty( $pb_layout ) || !empty( $content_areas ) ) ){
				$pb_layout[] = ZNB()->frontend->addModuleToLayout( 'TH_CustomSubHeaderLayout', array() , array(), false );
			}
			...
			if( !$has_content_and_sidebar && ( !empty($pb_layout) || !empty($content_areas ) ) ){
				...
				if( ! empty( $content_columns ) ){
					$pb_layout[] = ZNB()->frontend->addModuleToLayout( 'ZnSection', array( 'bottom_padding' => '0', 'top_padding' => '60' ) , $content_columns, false );
				}
			}

			if( !empty( $content_areas['content_main_area'] ) ){
				$pb_layout[] = ZNB()->frontend->addModuleToLayout( 'ZnSection', array( 'bottom_padding' => '0', 'top_padding' => '60' ) , $content_areas['content_main_area'], 'col-sm-12' );
			}
			...
			if( !empty( $content_areas['content_grey_area'] ) ){
				...
				$pb_layout[] = ZNB()->frontend->addModuleToLayout( 'ZnSection', array( 'css_class' => 'gray-area', 'top_padding' => '60','bottom_padding' => '20' ) , $content_areas['content_grey_area'], 'col-sm-12' );
			}

			// CONTENT BOTTOM AREA
			if( !empty( $content_areas['content_bottom_area'] ) ){
				$pb_layout[] = ZNB()->frontend->addModuleToLayout( 'ZnSection', array( 'bottom_padding' => '0' ) , $content_areas['content_bottom_area'], 'col-sm-12' );
			}
			...
			if( in_array( $meta_data->post_type, array( 'post', 'page' ) ) ){
				...
				if( !empty( $old_theme_options['zn_set_location_page_comments'] ) && $old_theme_options['zn_set_location_page_comments'] == 1 ){
					...
					if( comments_open( $meta_data->post_id ) ) {
						$comments_el = ZNB()->frontend->addModuleToLayout( 'TH_Comments', array() , false, false );
						$comments_column = array( ZNB()->frontend->addModuleToLayout( 'ZnColumn', array() , array( $comments_el ), 'col-sm-12' ) );
						$pb_layout[] = ZNB()->frontend->addModuleToLayout( 'ZnSection', array() , $comments_column, false );
					}
				}
			}
...
function zn_get_content_and_sidebar( $metadata, $meta_values,  $wrap_container = null, $pb_data = null ){
	...
	if( $wrap_container ){
		...
		if( !empty( $content_el ) ){
			$page_content_column = array( ZNB()->frontend->addModuleToLayout( 'ZnColumn', array() , $content_el , 'col-sm-12' ) );
		...
		$custom_container = ZNB()->frontend->addModuleToLayout( 'ZnCustomContainer', array() , $page_content_columns, false );
		$content_columns[] = ZNB()->frontend->addModuleToLayout( 'ZnColumn', array() , array( $custom_container ), $content_size );
	}
	elseif( !empty( $content_el ) ){
		$content_columns[] = ZNB()->frontend->addModuleToLayout( 'ZnColumn', array() , $content_el, $content_size );
	}
...
function zn_cnv_get_page_content( $metadata, $meta_values, $page_layout ){
	...
	switch ( $metadata->post_type ) {
		case 'post':
			...
			$content[] = ZNB()->frontend->addModuleToLayout( 'TH_PostContent', array() );
			break;
		case 'page':
			// Trebuie facut un check daca show_page_title == yes si daca avem content
			if( class_exists( 'WooCommerce' ) && wc_get_page_id( 'shop' ) == $metadata->post_id ){
				$archive_columns = $page_layout == 'no_sidebar' ? 4 : 3;
				$content[] = ZNB()->frontend->addModuleToLayout( 'TH_ProductArchive', array( 'num_columns' => $archive_columns ) );
			}
			elseif( $metadata->post_id == get_option( 'page_for_posts' ) ){
				// This is the page set as blog archive page
				$content[] = ZNB()->frontend->addModuleToLayout( 'TH_BlogArchive', array() );
			}
			elseif( ! empty( $metadata->post_content ) || ( !empty ( $meta_values['page_title_show'] ) && $meta_values['page_title_show'] == 'yes' )  ){
				$content[] = ZNB()->frontend->addModuleToLayout( 'TH_PageContent', array() );
			}
			break;
		case 'portfolio':
			$content[] = ZNB()->frontend->addModuleToLayout( 'TH_PortfolioContent', array() );
			break;
		case 'product':
			$content[] = ZNB()->frontend->addModuleToLayout( 'TH_ProductContent', array() );
			break;
		default:
	...
	if( in_array( $metadata->post_type, array( 'post', 'page' ) ) ){
		...
		if( isset( $old_theme_options['zn_set_location_page_comments'] ) && $old_theme_options['zn_set_location_page_comments'] === '0' ){
			...
			if( comments_open( $metadata->post_id ) ) {
				$content[] = ZNB()->frontend->addModuleToLayout( 'TH_Comments', array() , false, false );
			}
		}
	}
...
function zn_sidebar_config( $page_layout, $sidebar_select, $post_type ){
	...
	$sidebar_el = ZNB()->frontend->addModuleToLayout( 'TH_Sidebar', array( 'sidebar_select' => $sidebar_select ) );
	$return['sidebar_element'] = ZNB()->frontend->addModuleToLayout( 'ZnColumn', array() , array( $sidebar_el ), 'col-sm-3' );
