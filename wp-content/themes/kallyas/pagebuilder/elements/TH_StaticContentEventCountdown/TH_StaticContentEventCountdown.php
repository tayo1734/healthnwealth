...
echo	'		<form method="post" class="newsletter_subscribe newsletter-signup kl-newsletter clearfix" data-url="'.home_url('/').'" name="newsletter_form">';
echo	'			<input type="text" name="zn_mc_email" class="nl-email form-control kl-newsletter-field" value="" placeholder="'.esc_attr(__("your.address@email.com",'zn_framework')).'" />';
echo	'			<input type="hidden" name="zn_list_class" class="nl-lid" value="'.esc_attr($options['sc_ec_mlid']).'" />';
echo	'			<input type="submit" name="submit" class="nl-submit kl-newsletter-submit kl-font-alt btn btn-fullcolor" value="'.esc_html(__("JOIN US",'zn_framework')).'" />';
/*
